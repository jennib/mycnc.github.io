...
const App: React.FC = () => {
  ...
  const [editingMacroIndex, setEditingMacroIndex] = useState<number | null>(
    null
  );
  ...
  const [macros, setMacros] = useLocalStorage<Macro[]>(
    "cnc-app-macros",
    DEFAULT_MACROS
  );
  const [machineSettings, setMachineSettings] =
    useLocalStorage<MachineSettings>("cnc-app-settings", DEFAULT_SETTINGS);
  const [toolLibrary, setToolLibrary] = useLocalStorage<Tool[]>(
    "cnc-app-tool-library",
    []
  );
  const [generatorSettings, setGeneratorSettings] =
    useLocalStorage<GeneratorSettings>(
      "cnc-app-generator-settings",
      DEFAULT_GENERATOR_SETTINGS
    );
  ...
  useEffect(() => {
    ...
    if (
      hasSeenWelcome !== "true" &&
      (!isMachineSetupComplete || !isToolLibrarySetupComplete)
    ) {
  ...
  const playCompletionSound = useCallback(() => {
    ...
    if (audioBuffer && audioContext && audioContext.state === "running") {
      ...
      console.warn(
        "Could not play sound: AudioContext not running or sound buffer not loaded."
      );
    }
  }, []);
  ...
  const addLog = useCallback((log: { type: string; message: string }) => {
    ...
    if (
      processedLog.type === "error" &&
      processedLog.message.includes("error:")
    ) {
    ...
    setConsoleLogs((prev) => {
      ...
      if (
        !isVerboseRef.current &&
        processedLog.type === "received" &&
        trimmedMessage === "ok"
      ) {
  ...
  const handleConnect = useCallback(
    async (options: ConnectionOptions): Promise<void> => {
        ...
        onConnect: async (info: any) => {
          ...
          addLog({
            ...
            message: `Connected to ${
              options.type === "usb" ? "serial port" : "TCP"
            } at ${
              options.type === "usb"
                ? options.baudRate
      ...
      try {
        if (useSimulator) {
          ...
          // Use SerialManager for both USB and TCP
    ...
    [
      ...
      addNotification,
      playCompletionSound,
      machineSettings.scripts.startup,
      isVerboseRef,
    ]
  );
  ...
  const handleDisconnect = useCallback(async (): Promise<void> => {
    if (jobStatus === JobStatus.Running || jobStatus === JobStatus.Paused) {
      if (
        !window.confirm(
          "A job is currently running or paused. Are you sure you want to disconnect? This will stop the job."
        )
      ) {
    ...
    if (
      isConnected &&
      machineSettings.scripts.shutdown &&
      serialManagerRef.current
    ) {
      addLog({ type: "status", message: "Running shutdown script..." });
      const shutdownCommands = machineSettings.scripts.shutdown
        .split("\n")
        .filter((cmd) => cmd.trim() !== "");
  ...
  const handleFileLoad = (content: string, name: string): void => {
    ...
    addLog({
      type: "status",
      message: `Loaded ${name} (${lines.length} lines).`,
    });
  };
  ...
  const handleGCodeChange = (content: string): void => {
    ...
    addLog({
      type: "status",
      message: `G-code modified (${lines.length} lines).`,
    });
  };
  ...
  const handleLoadGeneratedGCode = useCallback(
    (gcode: string, name: string): void => {
  ...
  const handleJog = (axis: string, direction: number, step: number): void => {
    ...
    setIsJogging(true);
    serialManagerRef.current
      .sendLineAndWaitForOk(command)
      .catch((err: Error) => {
        const errorMessage =
          err instanceof Error ? err.message : "An error occurred during jog.";
  ...
  const handleSpindleCommand = useCallback(
    (command: "cw" | "ccw" | "off", speed: number): void => {
      const manager = serialManagerRef.current;
      if (!manager || !isConnected) return;
  ...
  const handleFeedOverride = useCallback(
    (command: "reset" | "inc10" | "dec10" | "inc1" | "dec1"): void => {
      const manager = serialManagerRef.current;
      if (!manager) return;
  ...
  const handleUnitChange = useCallback(
    (newUnit: "mm" | "in"): void => {
      ...
      addLog({
        type: "status",
        message: `Units set to ${newUnit === "mm" ? "millimeters" : "inches"}.`,
      });
  ...
  const handleProbe = useCallback(
    async (axes: "X" | "Y" | "Z" | "XYZ"): Promise<void> => {
      const manager = serialManagerRef.current;
      ...
      addLog({
        type: "status",
        message: `Starting ${axes.toUpperCase()}-Probe cycle...`,
      });
      ...
      try {
        const probeAxis = async (
          ...
          await manager.sendLineAndWaitForOk(
            `G38.2 ${axis}${travel.toFixed(4)} F${probeFeed}`
          );
          addLog({
            type: "status",
            message: `Probe contact detected on ${axis}.`,
          });
          await manager.sendLineAndWaitForOk(`G10 L20 P1 ${axis}${offset}`);
          addLog({
            type: "status",
            message: `${axis}-axis zero set to ${offset}${unit}.`,
          });
          await manager.sendLineAndWaitForOk("G91");
          await manager.sendLineAndWaitForOk(
            `G0 ${axis}${retractDist * -travelDir}`
          );
        ...
        addNotification(
          `${axes.toUpperCase()}-Probe cycle complete.`,
          "success"
        );
      } catch (error) {
        const errorMessage =
        ...
        addLog({
          type: "error",
          message: `Probe cycle failed: ${errorMessage}`,
        });
  ...
  const handleHome = useCallback(
    (axes: "all" | "x" | "y" | "z" | "xy"): void => {
      ...
      addLog({
        type: "status",
        message: `Starting homing cycle for: ${axes.toUpperCase()}...`,
      });
  ...
  const handleSetZero = useCallback(
    (axes: "all" | "x" | "y" | "z" | "xy"): void => {
      ...
      addLog({
        type: "status",
        message: `Work coordinate origin set for ${axes.toUpperCase()}.`,
      });
    },
    [addLog]
  );
  ...
  const handleRunMacro = useCallback(
    async (commands: string[]): Promise<void> => {
      const manager = serialManagerRef.current;
      if (!manager) return;
      ...
      addLog({
        type: "status",
        message: `Running macro: ${processedCommands.join("; ")}`,
      });
      try {
  ...
  const handleExportSettings = useCallback((): void => {
    ...
    const url = URL.createObjectURL(
      new Blob([JSON.stringify(settingsToExport, null, 2)], {
        type: "application/json",
      })
    );
    a.href = url;
    a.download = `mycnc-app-settings-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
  ...
  const handleImportSettings = useCallback(
    (imported: any): void => {
      if (
        !window.confirm(
          "This will overwrite your current macros, settings, tool library, and generator settings. Are you sure?"
        )
      ) {
        return;
      }
  ...
  const isElectron =
    (process.env.IS_ELECTRON === "true" || !!window.electronAPI?.isElectron);
  console.log("App.tsx: isElectron =", isElectron); // Add this line
  ...
  useEffect(() => {
    const handleBeforeUnload = (event: BeforeUnloadEvent): string | void => {
      if (isJobActive) {
        ...
        event.returnValue =
          "A job is currently active. Are you sure you want to leave?"; // Required for Chrome, triggers confirmation dialog
  ...
  const isAnyControlLocked =
    !isConnected ||
    isJobActive ||
    isJogging ||
    isMacroRunning ||
    (machineState?.status && ["Alarm", "Home"].includes(machineState.status));
  const selectedTool =
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-text-primary flex flex-col">
      ...
      <WelcomeModal
        ...
        onClose={() => {
          const isMachineSetupComplete =
            machineSettings.workArea.x > 0 && machineSettings.workArea.y > 0;
          const isToolLibrarySetupComplete = toolLibrary.length > 0;
          const hasSeenWelcome = localStorage.getItem("cnc-app-seen-welcome");
          ...
          if (isMachineSetupComplete && isToolLibrarySetupComplete) {
            ...
            addNotification(
              "Please complete all setup tasks to dismiss this window.",
              "info"
            );
          }
        }}
      ...
      <header className="bg-surface shadow-md p-4 flex justify-between items-center z-10 flex-shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          >
            ...
            >
              ...
              <path
                d="M 0,-48 A 48,48 0 0 1 30,16 L 10,6 A 12,12 0 0 0 0,-12 Z"
        ...
        <div className="flex items-center gap-4">
          ...
          />
          {console.log("App.tsx: isElectron prop passed to SerialConnector:", isElectron)}
        </div>
      </header>
      ...
      {isAlarm && (
        ...
        >
          ...
          <div className="flex-grow">
            <h3 className="font-bold text-lg">{`Machine Alarm: ${
              alarmInfo!.name
            }`}</h3>
